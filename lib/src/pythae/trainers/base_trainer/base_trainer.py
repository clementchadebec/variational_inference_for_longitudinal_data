...
class BaseTrainer:
    ...
    def _optimizers_step(self, model_output=None):
        ...
        #    torch.nn.utils.clip_grad_norm(self.model.parameters(), self.training_config.gradient_clip)
    ...
    def train(self, log_output_dir: str = None):
        ...
        for epoch in range(1, self.training_config.num_epochs + 1):
            ...
            #    import itertools
            #    self.optimizer = torch.optim.Adam(itertools.chain(self.model.flows.parameters(), self.model.decoder.parameters()), lr=self.training_config.learning_rate)
    ...
    def predict(self, model: BaseAE, epoch=None):
        ...
        model_out = model(inputs, epoch=epoch)
        reconstructions = model_out.recon_x.cpu().detach()
        z_enc = model_out.z.detach()
        x_seen = model_out.x.detach()
        ...
        with torch.no_grad():
            if model.model_name == "LVAE_IAF": 
                if model.prior == "standard":
                    ...
                    if model.vamp_number_components < 10:
                        ...
                        for i in range(int(10 / model.vamp_number_components)+1):
                            means.append(
                                model.pseudo_inputs(
                                    model.idle_input.to(z_enc.device)
                        ...
                        means = model.pseudo_inputs(model.idle_input.to(z_enc.device))[
                                    :10
                ...
                z = torch.randn(10*x_seen.shape[1], model.latent_dim).cuda()

            elif model.model_name == "VAMP":
                    if model.number_components < 10*x_seen.shape[1]:
                        means = []
                        for i in range(int(10*x_seen.shape[1] / model.number_components)+1):
                            means.append(
                                model.pseudo_inputs(
                                    model.idle_input.to(z_enc.device)
                                ).reshape((model.number_components,) + model.model_config.input_dim))

                        means = torch.cat(means)[:10*x_seen.shape[1]].reshape((10*x_seen.shape[1],) + model.model_config.input_dim)

                    else:
                        means = model.pseudo_inputs(model.idle_input.to(z_enc.device))[
                                    :10*x_seen.shape[1]
                                ].reshape((10*x_seen.shape[1],) + model.model_config.input_dim)
            ...
            if model.model_name == "LVAE_IAF":
                normal_generation = model.generate(z).detach().cpu()
            elif model.model_name == "GPVAE":
                pz = model._get_prior()
                z = pz.sample((10,))
                z = torch.transpose(z, 1, 2)
                normal_generation = model.decoder(z.reshape(-1, model.latent_dim))["reconstruction"].detach().cpu().reshape((10, x_seen.shape[1])+model.model_config.input_dim)
            else:
                normal_generation = model.decoder(z).reconstruction.detach().cpu().reshape((10, x_seen.shape[1])+model.model_config.input_dim)
        print(normal_generation.shape)
        return inputs["data"], x_seen, reconstructions, normal_generation, inputs['seq_mask'], inputs['pix_mask']
